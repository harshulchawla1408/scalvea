-- 1. Enforce strict market isolation constraints on checkout_sessions
ALTER TABLE public.checkout_sessions DROP CONSTRAINT IF EXISTS chk_market_currency;
ALTER TABLE public.checkout_sessions
ADD CONSTRAINT chk_market_currency
CHECK (
  (market = 'AU' AND currency = 'AUD') OR
  (market = 'IN' AND currency = 'INR')
);

-- 2. Enforce strict market isolation constraints on orders
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS chk_orders_market_currency;
ALTER TABLE public.orders
ADD CONSTRAINT chk_orders_market_currency
CHECK (
  (market = 'AU' AND currency = 'AUD') OR
  (market = 'IN' AND currency = 'INR')
);

-- 3. RPC Function for Transactional Order Processing with Market Verification
...
BEGIN
    ...
    -- 1a. MARKET ISOLATION VALIDATION
    IF p_order_source = 'Stripe' AND (v_session.market != 'AU' OR v_session.currency != 'AUD') THEN
        RETURN jsonb_build_object('success', false, 'error', 'Invalid market or currency for Stripe. Must be AU/AUD.');
    END IF;

    IF p_order_source = 'SHIPROCKET' AND (v_session.market != 'IN' OR v_session.currency != 'INR') THEN
        RETURN jsonb_build_object('success', false, 'error', 'Invalid market or currency for Shiprocket. Must be IN/INR.');
    END IF;
    ...
    LOOP
        ...
        IF v_current_inventory < (v_item->>'quantity')::int THEN
            RAISE EXCEPTION 'Insufficient inventory for product % (Requested: %, Available: %)', v_item->>'name', v_item->>'quantity', v_current_inventory;
        END IF;
        ...
        INSERT INTO public.inventory_logs (product_id, change_amount, previous_quantity, new_quantity, reason)
        VALUES (
            (v_item->>'productId')::uuid,
            -(v_item->>'quantity')::int,
            v_current_inventory,
            v_current_inventory - (v_item->>'quantity')::int,
            'Order ' || v_order_id || ' placed'
        );

        -- Insert Order Item
        INSERT INTO public.order_items (
            order_id, product_id, product_name, sku, variant, quantity, price, image_url, currency
        ) VALUES (
            v_order_id,
            (v_item->>'productId')::uuid,
            v_item->>'name',
    ...
    -- 6. Audit Logging
    INSERT INTO public.order_audit_logs (order_id, event_type, description)
    VALUES 
    (v_order_id, 'ORDER_CREATED', 'Order created successfully via ' || p_order_source),
    (v_order_id, 'PAYMENT_CONFIRMED', 'Payment confirmed (' || COALESCE(p_payment_details->>'transaction_id', 'N/A') || ')');

    RETURN jsonb_build_object('success', true, 'order_id', v_order_id);
EXCEPTION WHEN OTHERS THEN
    -- In Postgres, any unhandled exception in a function rolls back the transaction.
    -- We can log it to system logs but we must re-raise or return failure.
    -- Note: Writing to system_logs here inside the exception block will ALSO be rolled back!
    -- Therefore, edge functions must catch the failure and insert into system_logs.
