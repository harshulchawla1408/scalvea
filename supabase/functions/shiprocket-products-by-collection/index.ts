...
serve(async (req) => {
  ...
  try {
    ...
    // Find the category matching the collection ID
    const uniqueCategories = Array.from(new Set((allProducts || []).map((p: any) => p.category || "General")));
    const matchedCategory = uniqueCategories.find((cat) => getCollectionId(cat) === targetCollectionId);

    if (!matchedCategory) {
      // If collection_id doesn't match any category, return empty list
      return new Response(
        JSON.stringify({
          data: {
            total: 0,
    ...
    // Now query products in that category
